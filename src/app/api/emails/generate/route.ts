...
import { EmailType } from "@/lib/openai/types";
...
function mapEmailType(frontendType: string): EmailType {
  const typeMap: { [key: string]: EmailType } = {
  ...
  try {
    ...
    if (!contactName || !jobTitle || !companyName || !emailType) {
      return NextResponse.json(
        {
          error:
            "Contact name, job title, company name, and email type are required",
        },
        { status: 400 },
      );
    }

    // Validate emailType is a valid EmailType
    ...
    const { data: userProfile, error: profileError } = await supabase
      .from("users")
      .select(
        "first_name, last_name, university, study_level, field_of_study, phone, linkedin",
      )
      .eq("id", user.id)
      .single();
    ...
    if (!existingEmailRecord) {
      console.error(
        "❌ No email address record found. Generate email address first.",
      );
      return NextResponse.json(
        {
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        { error: "Failed to save email content" },
        { status: 500 },
      );
    }
