...
export default function ProfilePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      // ✅ NEW: Use the API instead of direct Supabase calls
      ...
      // ✅ You can add an error toast here later
      // For now, you could set an error state to show to user
  ...
  const createdAt = profile?.created_at ? new Date(profile.created_at).toLocaleDateString('fr-FR') : 'N/A';
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center space-x-3">
                  <div className={`p-2 rounded-lg ${currentIsPremium ? 'bg-yellow-100' : 'bg-gray-100'}`}>
                    {currentIsPremium ? (
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center space-x-3">
                  <div className={`p-2 rounded-lg ${
                    currentIsPremium ? 'bg-green-100' : 
                    creditsRemaining <= 1 ? 'bg-red-100' : 
                    creditsRemaining <= 2 ? 'bg-yellow-100' : 'bg-blue-100'
                  }`}>
                    <Mail className={`h-4 w-4 ${
                      currentIsPremium ? 'text-green-600' : 
                      creditsRemaining <= 1 ? 'text-red-600' : 
                      creditsRemaining <= 2 ? 'text-yellow-600' : 'text-blue-600'
                    }`} />
                  </div>
                  <div>
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      {!currentIsPremium && creditsRemaining <= 2 && (
                        <span className={`text-xs px-2 py-1 rounded-full ${
                          creditsRemaining === 0 ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'
                        }`}>
