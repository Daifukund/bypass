    ...
    try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a LinkedIn content analyzer. Return only valid JSON arrays with employee data. Extract real people with names, job titles, and locations from the provided LinkedIn content.",
          },
          {
            role: "user",
            content: prompt,
      ...
      if (!Array.isArray(rawEmployees) || rawEmployees.length === 0) {
        return NextResponse.json({
          ...
          total: 0,
          message:
            "No employees found in the provided content. Please make sure you copied the LinkedIn people search results correctly.",
        });
      }
