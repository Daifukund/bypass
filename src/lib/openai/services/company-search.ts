...
export class CompanySearchService {
  ...
  static async searchCompanies(
    ...
    try {
      ...
      if (searchMode === 'websearch' && supportsWebSearch()) {
        ...
        return await this.searchWithWebSearch(sanitizedCriteria, prompt, requestId);
      } else if (searchMode === 'standard') {
        console.log('⚡ Using Standard mode (user selected)');
        return await this.searchWithStandardCompletion(sanitizedCriteria, prompt, requestId);
      } else {
  ...
  static async searchWithCustomPrompt(
    ...
    try {
      if (useWebSearch && supportsWebSearch()) {
        ...
        const content = response.choices[0]?.message?.content;
        if (!content) {
          throw new Error('No response content from custom standard completion');
        }
   ...
   */
  static validateCriteria(criteria: SearchCriteria): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    const hasJobTitle = criteria.jobTitle || criteria.job_title;
    const hasLocation = criteria.location;
    const hasIndustry = criteria.industry;
    const hasKeywords = criteria.keywords && (
      (Array.isArray(criteria.keywords) && criteria.keywords.length > 0) ||
      (typeof criteria.keywords === 'string' && criteria.keywords.trim().length > 0)
    );

    if (!hasJobTitle && !hasLocation && !hasIndustry && !hasKeywords) {
      errors.push('At least one search parameter is required (job title, location, industry, or keywords)');
    }

    // Validate job title length if provided
    if (hasJobTitle) {
      const jobTitle = criteria.jobTitle || criteria.job_title || '';
