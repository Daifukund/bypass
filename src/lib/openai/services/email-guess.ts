...
export class EmailGenerationService {
   ...
   */
  static async generateEmailAddress(fullName: string, companyName: string): Promise<EmailData> {
  ...
  console.log('🔍 EMAIL_ADDRESS_GENERATION type:', typeof OPENAI_PROMPTS.EMAIL_ADDRESS_GENERATION);
  console.log('🔍 EMAIL_ADDRESS_GENERATION value:', OPENAI_PROMPTS.EMAIL_ADDRESS_GENERATION);
  
  // Check if the function exists before calling it
  // Direct function definition to bypass import issue


  const prompt = OPENAI_PROMPTS.EMAIL_ADDRESS_GENERATION(fullName, companyName);
console.log('🔍 Generated prompt:', prompt.substring(0, 200) + '...');
    ...
    try {
      ...
      try {
        return await this.generateWithStandardCompletion(criteria, prompt, requestId);
      } catch (fallbackError) {
   ...
   */
  private static generateBasicEmailGuess(fullName: string, companyName: string): EmailData {
    ...
    return {
      email: email,
      confidence_score: 0.75 // Reasonable confidence for common format
    };
  }
  ...
  private static async generateWithWebSearch(
    ...
    try {
      ...
      return this.generateBasicEmailGuess(criteria.fullName, criteria.companyName);
    }
