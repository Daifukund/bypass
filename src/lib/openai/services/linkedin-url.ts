...
export class LinkedInUrlService {
   ...
   */
  static async generateLinkedInUrl(criteria: LinkedInUrlCriteria): Promise<LinkedInUrlResult> {
    ...
    try {
      ...
      if (supportsWebSearch()) {
        return await this.generateWithWebSearch(criteria, prompt, requestId, fallbackUrl);
      } else {
        console.log('⚠️ Web search not available, using standard completion');
        return await this.generateWithStandardCompletion(criteria, prompt, requestId, fallbackUrl);
      }
    } catch (error) {
  ...
  private static async generateWithWebSearch(
    ...
    console.log('📤 LinkedIn URL Web Search Request:', JSON.stringify(requestPayload, null, 2));
    ...
    if (rawData && typeof rawData === 'object' && 'linkedin_people_search_url' in rawData) {
      const generatedUrl = (rawData as any).linkedin_people_search_url;
      if (generatedUrl && typeof generatedUrl === 'string' && generatedUrl.includes('linkedin.com')) {
  ...
  private static async generateWithStandardCompletion(
    ...
    console.log('📤 LinkedIn URL Standard Completion Config:', JSON.stringify(config, null, 2));
