...
export async function POST(req: NextRequest) {
  try {
    ...
    const aiUrl = await generateAILinkedInUrl(
      body.companyName, 
      body.jobTitle, 
      body.location
    );

    // TESTING: Only use AI URL, no fallback
    if (!aiUrl) {
      return NextResponse.json({
        success: false,
        error: 'OpenAI WebSearch failed and fallback is disabled for testing',
        message: 'No LinkedIn URL generated - WebSearch failed'
      }, { status: 500 });
    }
